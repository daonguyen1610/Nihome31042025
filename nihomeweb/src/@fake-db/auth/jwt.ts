...
const users: UserDataType[] = [
  {
    id: 1,
    role: 'admin',
    password: 'nihome-admin',
    fullName: 'Nihome Admin',
    username: 'nihome-admin',
    email: 'admin@nihome.local'
  },
  {
    id: 2,
    role: 'client',
    password: 'nihome-client',
    fullName: 'Nihome Client',
    username: 'nihome-client',
    email: 'client@nihome.local'
  }
]

// Keep the mock auth flow runnable in fresh environments even before local envs are added.
const jwtConfig = {
  secret: process.env.NEXT_PUBLIC_JWT_SECRET?.trim() || 'nihome-dev-secret',
  expirationTime: process.env.NEXT_PUBLIC_JWT_EXPIRATION?.trim() || '5m',
  refreshTokenSecret:
    process.env.NEXT_PUBLIC_JWT_REFRESH_TOKEN_SECRET?.trim() || 'nihome-dev-refresh-secret'
}
