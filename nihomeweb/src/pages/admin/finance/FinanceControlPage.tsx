...
const FinanceControlPage = () => {
  ...
  const [lookupAvailable, setLookupAvailable] = useState({ contracts: true, vendors: true, projects: true, users: true });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      adminApi.listVendors({ isActive: true, page: 1, pageSize: 200 }).then((response) => response.data.items).catch(() => null),
    ...
    ]).then(([contractRows, vendorRows, projectRows, userRows]) => {
      if (cancelled) return;
      setLookupAvailable({ contracts: contractRows != null, vendors: vendorRows != null, projects: projectRows != null, users: userRows != null });
    ...
    <AdminLayout>
      ...
      <Dialog open={dialog != null} onOpenChange={(open) => { if (!open && !busy) setDialog(null); }}>
        <DialogContent className="max-h-[92vh] w-[95vw] max-w-3xl overflow-y-auto">
          ...
          {dialog === "payment" && <PaymentForm form={paymentForm} setForm={setPaymentForm} contracts={contracts} vendors={vendors} accountants={accountants} lookupAvailable={lookupAvailable} t={t} />}
          ...
          <DialogFooter><Button variant="outline" onClick={() => setDialog(null)} disabled={busy}>{t("common.cancel")}</Button><Button onClick={dialog === "payment" ? createPayment : dialog === "period" ? createPeriod : createCorrection} disabled={busy}>{busy ? t("common.saving") : t("finance.action.create")}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
...
const PaymentForm = ({ form, setForm, contracts, vendors, accountants, lookupAvailable, t }: { form: ReturnType<typeof emptyPayment>; setForm: React.Dispatch<React.SetStateAction<ReturnType<typeof emptyPayment>>>; contracts: ContractResponse[]; vendors: VendorResponse[]; accountants: UserOption[]; lookupAvailable: { contracts: boolean; vendors: boolean; users: boolean }; t: Translate }) => {
