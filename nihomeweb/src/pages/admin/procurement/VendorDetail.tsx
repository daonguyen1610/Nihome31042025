      ...
      <div className="space-y-5 p-4 sm:p-6">
        ...
        {loading ? <PageLoading /> : error || !vendor ? <PageError message={error || t("proc.vendors.notFound")} onRetry={() => void load()} /> : (
          <>
            ...
            <section className="space-y-3"><h2 className="text-lg font-semibold">{t("proc.vendors.section.documents")}</h2><div className="flex flex-wrap gap-2 rounded-md border bg-card p-5">{capabilityUrl ? <Button asChild variant="outline"><Link to={capabilityUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-2 h-4 w-4" />{t("proc.vendors.openCapability")}</Link></Button> : <p className="text-sm text-muted-foreground">{t("proc.vendors.noDocuments")}</p>}{folderUrl && <Button asChild variant="outline"><Link to={folderUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-2 h-4 w-4" />{t("proc.vendors.openFolder")}</Link></Button>}</div></section>
