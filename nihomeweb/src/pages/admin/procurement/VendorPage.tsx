...
import { adminApi, type CreateVendorRequest, type DeletionImpactResponse, type UpdateVendorRequest, type VendorListParams, type VendorResponse, type VendorType } from "@/services/adminApi";
...
export default function VendorPage() {
  ...
  const openDelete = async (vendor: VendorResponse) => {
  ...
  const deleteVendor = async (confirmation: string) => {
    ...
    setDeleteError(null);
    try {
      return (await adminApi.deleteVendor(pendingDelete.id, {
        planToken: deleteImpact!.planToken,
        confirmation,
        rowVersion: pendingDelete.rowVersion,
      })).data;
    } catch (deleteError) {
      setDeleteError(extractApiError(deleteError) || t("common.error"));
      throw deleteError;
    } finally {
      setDeleting(false);
    }
  ...
  return (
    <AdminLayout>
      ...
      <DeletionImpactDialog open={pendingDelete !== null} impact={deleteImpact} loading={loadingImpact} deleting={deleting} error={deleteError} onOpenChange={(open) => { if (!open && !deleting) setPendingDelete(null); }} onConfirm={deleteVendor} onCompleted={deleteCompleted} />
    </AdminLayout>
  );
}