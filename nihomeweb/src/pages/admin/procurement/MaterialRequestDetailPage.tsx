...
const MaterialRequestDetailPage = () => {
  ...
  const lifecycle = [
    ...
    { label: t("procurement.request.history.cancelled"), value: request.cancelledAt, actor: null },
    ...
    <AdminLayout>
      <div className="space-y-6 p-4 sm:p-6">
        ...
        <header className="flex flex-col gap-4 border-b pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <Button asChild variant="ghost" size="sm" className="mb-2 -ml-3"><Link to="/admin/procurement-control"><ArrowLeft className="mr-2 h-4 w-4" />{t("common.back")}</Link></Button>
      ...
      <Dialog open={cancelOpen} onOpenChange={(open) => { if (!open && !busy) setCancelOpen(false); }}><DialogContent><DialogHeader><DialogTitle>{t("procurement.request.cancel.title")}</DialogTitle><DialogDescription>{t("procurement.request.cancel.description")}</DialogDescription></DialogHeader><ReasonField value={reason} setValue={setReason} t={t} />{formError && <p className="text-sm text-destructive" role="alert">{formError}</p>}<DialogFooter><Button variant="outline" onClick={() => setCancelOpen(false)} disabled={busy}>{t("common.back")}</Button><Button variant="destructive" onClick={cancel} disabled={busy}>{busy ? t("common.saving") : t("procurement.action.cancelRequest")}</Button></DialogFooter></DialogContent></Dialog>
    </AdminLayout>
  );
};
