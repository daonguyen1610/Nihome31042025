        ...
        <SheetContent className="w-full overflow-y-auto sm:max-w-2xl">
          ...
          {detailLoading ? <PageLoading /> : detailError ? <PageError message={detailError} /> : detail && (
            <div className="mt-6 space-y-6">
              ...
              <section><h3 className="mb-2 text-sm font-semibold">{t("handover.field.documents")}</h3>{detail.documents.length === 0 ? <p className="text-sm text-muted-foreground">{emptyValue}</p> : <div className="space-y-2">{detail.documents.map((documentUrl, index) => { const href = resolveSafeLinkUrl(documentUrl); return href ? <a key={`${documentUrl}-${index}`} href={href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 break-all text-sm text-primary hover:underline"><ExternalLink className="h-4 w-4 shrink-0" />{documentUrl}</a> : <p key={`${documentUrl}-${index}`} className="break-all text-sm text-muted-foreground">{documentUrl}</p>; })}</div>}</section>
