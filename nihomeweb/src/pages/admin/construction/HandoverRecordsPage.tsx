    ...
    <AdminLayout>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
          ...
          <section className="space-y-3"><div className="flex items-center justify-between"><Label>{t("handover.field.documents")}</Label><Button type="button" size="sm" variant="outline" disabled={form.documents.length >= 20} onClick={() => setForm((current) => ({ ...current, documents: [...current.documents, ""] }))}><Plus className="mr-2 h-4 w-4" />{t("handover.form.addDocument")}</Button></div><AdminDocumentUpload multiple maxFiles={Math.max(0, 20 - form.documents.filter((item) => item.trim()).length)} uploadFile={async (file) => (await adminApi.uploadHandoverDocument(file)).data.path} onUploaded={(paths) => setForm((current) => ({ ...current, documents: [...current.documents, ...paths] }))} disabled={saving} testId="handover-document-upload" />{form.documents.map((item, index) => <div key={index} className="flex gap-2"><Input maxLength={500} value={item} onChange={(event) => updateStringRow("documents", index, event.target.value)} placeholder={t("handover.form.documentPlaceholder")} />{item.trim() && (!isManagedDocumentPath(item, "/files/business-documents/handover") || rows.find((row) => row.id === editingId)?.documents.includes(item)) && <AdminFilePreview url={item} fileName={item.split("/").pop()} fetchFile={isManagedDocumentPath(item, "/files/business-documents/handover") ? async () => (await adminApi.getHandoverDocumentContent(editingId!, item)).data : undefined} />}<Button type="button" size="icon" variant="ghost" onClick={() => removeRow("documents", index)} aria-label={t("handover.action.remove")}><Trash2 className="h-4 w-4" /></Button></div>)}</section>
