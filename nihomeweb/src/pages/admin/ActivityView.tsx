...
import { ArrowLeft, Edit, Trash2, Calendar, Grid3X3, List, Tag, User } from "lucide-react";
    ...
    return (
      <AdminLayout>
        <div className="admin-card p-10 text-center">
          <p className="text-sm" style={{ color: "hsl(var(--admin-muted))" }}>{t("activities.notFound")}</p>
          <Link to="/admin/activities" className="admin-btn-primary inline-flex items-center gap-2 px-5 py-2.5 text-sm mt-4">
    ...
    <AdminLayout>
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2 shrink-0">
          <Link
            to={`/admin/activities/${post.slug}/edit`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="admin-card p-6 h-fit space-y-3 text-sm">
          ...
          <Info icon={Tag} label={t("activities.field.category")} value={post.category} />
          <Info icon={User} label={t("activities.field.author")} value={post.author ?? "—"} />
          <Info icon={Calendar} label={t("activities.field.date")} value={post.date} />
        </div>
      </div>
    </AdminLayout>
  );
