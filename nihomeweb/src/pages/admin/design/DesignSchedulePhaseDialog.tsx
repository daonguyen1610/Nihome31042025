    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div><Label htmlFor="phase-weight">{t("designProjects.schedule.field.weight")}</Label><Input id="phase-weight" className="mt-1 min-h-11" type="number" min={1} max={100} value={form.weight} onChange={(event) => setForm({ ...form, weight: Number(event.target.value) })} /></div>
        </div>
