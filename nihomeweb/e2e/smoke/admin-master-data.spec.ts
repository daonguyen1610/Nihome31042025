...
test("SPA renders /admin/master-data without console errors for SUPER_ADMIN", async ({
    ...
    // A category picker + at least one row means the API round trip
    // succeeded and the seeded catalogue rendered. Playwright uses Desktop
    // Chrome (≥ lg breakpoint) so the table variant is the visible one.
