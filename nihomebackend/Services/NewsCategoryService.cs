...
public class NewsCategoryService(AppDbContext db, ILogger<NewsCategoryService> logger)
{
    ...
    public async Task<NewsCategoryResponse?> UpdateAsync(int id, UpsertNewsCategoryRequest req)
    {
        ...
        var explicitLanguages = await LocalizedCategoryTranslationSync.GetExplicitLanguagesAsync(db, EntityTypes.NewsCategory, id);
        var nameVi = NormalizeName(!string.IsNullOrWhiteSpace(req.NameVi) ? req.NameVi : req.Name);
    ...
    public async Task<bool> DeleteAsync(int id)
    {
        ...
        db.EntityTranslations.RemoveRange(db.EntityTranslations.Where(translation =>
            translation.EntityType == EntityTypes.NewsCategory && translation.EntityId == id));
